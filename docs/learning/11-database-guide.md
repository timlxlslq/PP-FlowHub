...
先给结论：项目已经有统一的数据库文件路径和 schema 初始化入口，但没有一个全项目通用的 `get_connection()` 工厂。独立的同步入口可以使用 `Config.workflow_database` 得到文件路径，再显式调用 `sqlite3.connect(...)`。保存函数更适合显式接收调用方传入的 `connection`，由调用方决定事务边界、提交、回滚和关闭。这样可以把一次同步中的多个写入放进同一个短事务，也不会让底层函数误关闭或误提交借来的连接。
...
`ensure_schema` 会创建父目录、打开自己的临时连接、创建或升级它负责的业务表、提交并关闭这个连接。它不是一个“返回已经打开连接”的函数。
...
- `ensure_schema(path)` 负责当前数据库的建表和轻量 schema 升级；它不是旧数据业务迁移器。
...
| 入口 | 签名或形式 | 作用 | 连接/事务行为 |
| --- | --- | --- | --- |
| [`database_path`](../../traveler_assistant/database.py#L19) | `database_path(state_dir: Path) -> Path` | 从状态目录得到 `workflow.sqlite3` | 不打开连接 |
| [`ensure_schema`](../../traveler_assistant/database.py#L138) | `ensure_schema(path: Path) -> None` | 创建和升级中央 schema | 自己连接、提交、关闭 |
...
| 订单材料 | `material_items` | 以 `order_id` 和材料属性保存订单级材料事实 |
| 工厂单五金 | `hardware_items` | 通过 `order_id`、`factory_order` 归属工厂单，`source_type` 区分来源 |
| 生产批次 | `manual_production_batches` | `batch_id`/`batch_number` 表示一次生产记录 |
| 生产关联 | `manual_production_batch_factories`、`manual_production_batch_materials` | 通过 `batch_id` 关联工厂单和本次消耗材料 |
...
2. `get_records(..., limit=100)` 的 `100` 只是单次请求上限，不代表已经取得全量数据。分页字段、排序和继续读取条件必须根据 CLI 返回契约确认。
...
当前 [`traveler_assistant/wecom_service.py`](../../traveler_assistant/wecom_service.py) 有两个读取入口：`get_smartsheet_info(doc_url)` 和 `get_records(doc_url, sheet_id, limit=100)`；没有写入入口。底层 `_run_wecom_command(args)` 负责运行 `wecom-cli`、解析 JSON 和抛出外部命令错误。
...
1. 先读 [`core.py#L124`](../../traveler_assistant/core.py#L124) 的 `Config`，理解 `state_dir`、`workflow_database` 和 `prepare_storage()`。
2. 再读 [`database.py#L19`](../../traveler_assistant/database.py#L19) 的 `database_path`、[`database.py#L138`](../../traveler_assistant/database.py#L138) 的 `ensure_schema`，区分“路径”“初始化”和“连接”。
3. 读 [`order_index.py#L703`](../../traveler_assistant/order_index.py#L703) 的 `OrderIndexStore`，确认订单/工厂单建表职责和可选的显式连接传递。
4. 读 [`order_service.py#L41`](../../traveler_assistant/order_service.py#L41) 的 `serve()`，理解常驻服务如何打开并注册一条共享连接。
5. 读 [`production.py#L430`](../../traveler_assistant/production.py#L430) 的 `record_completed_production`，观察“函数接收 connection、调用方决定事务”的写法。
...
需要查看当前两个正式 SQLite 运行库的完整字段、类型、可空、默认值、约束和源码证据时，参见[当前数据库字段字典](database-map/current-schema.md)；文件角色与按表查找入口见[数据库地图](12-database-map.md)。
