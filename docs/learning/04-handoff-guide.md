...
复杂功能、跨模块修改、重大重构或明显架构取舍任一成立时，优先更新与当前任务相关的已有计划，确需新建时放在 `docs/plans/`；简单修改不建计划，不为形式新建空目录。计划管理不依赖特定插件或技能。计划随任务更新，文首记录目标、状态、约束、步骤、验证、完成项和剩余决策。

## 阅读范围速查

测试时区分“开发反馈”和“发布证据”：改 AIMES 的某个过滤条件，可以先运行对应 unittest 方法，快速知道该行为是否正确；代码稳定后由 `test-release` 完成全部必需回归。已经通过且输入未变的检查，不必为了安心反复运行。测试预期应来自业务要求，例如“全部已生产仍读取最近 50 条”，而不是断言实现里恰好出现某个字符串。布局、点击和草稿放弃仍需实际界面验证。具体命令与收口规则见 [发布测试文档](../release-testing.md#开发阶段如何安排测试)。
...
本项目不单独展示或维护构建号。小修改默认用 `build-app` 将 `0.4.1` 升为 `0.4.2`；新增或修改较大功能用 `build-app --minor` 将版本升为 `0.5.0`，末位归零。`tools/app_version.py` 按数字比较源码、上次产物和安装版，计算最高版本的下一版；通过打包检查后，把同一版本写入 `macos/Info.plist` 和 App 包的两个系统版本字段。`install-app` 只签名安装，不再次递增。

SwiftUI 的 `WindowGroup` 标题从 `Bundle.main` 读取当前运行包，显示 `PP FlowHub  V0.4.2`；`V` 不写入系统版本字段。旧进程仍显示旧号，退出并重开安装版才会显示新号。这是项目文件与包元数据的变化，不需要业务数据库迁移。下一步可对照源码 plist、构建包和安装包，理解“源码配置”和“正在运行的程序”为什么是不同证据。
