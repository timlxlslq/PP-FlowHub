...
常驻进程启动后，不会自动获得 App 修改后的环境变量。因此每次 `ResidentOrderServiceClient.request()` 都携带 `operation_log_enabled`；Python `order_service.serve()` 在处理请求和缓存命中前同步 logger、config 及环境开关。更新配置而不重启进程，能够保留正在执行的任务。

未知异常经 `safe_exception_details()` 提取有限代码位置：模块、函数和行号足以找到源码，不需要记录可能含密码或业务输入的局部变量。查看器用 `OperationLogReader.read()` 区分空日志、读取错误和坏行，用 `filtered()` 筛选，用 `diagnosticText(for:in:)` 按会话和操作双键复制上下文。这是“原始记录—安全读取模型—界面呈现”的分工；日志可以解释执行过程，但不能替代数据库事务提交或外部单据事实。
...
关键边界：`sync_aimes_index()` 只刷新 AIMES 身份，不扫描 Server。格式异常行只保存在连接内的 `pending_aimes_reviews` 临时表，并由读层投影为本次会话待人工确认的警告；它跳过有效业务写入。失败时可以返回缓存，但缓存不能表述成刚刚在线验证成功。

## 7. Server 扫描、预览与确认

### 新版入口：优化编号只处理一次

`scan-server → aicnc_import.discover` 只找新编号；`preview-server-changes → aicnc_import.preview` 读取报表并给出工厂单用途建议；`AicncConfirmationSheet → confirm-aicnc` 以最终分类、材料分配和五金选择确认。`ignore-aicnc` 只忽略一个编号，`dismiss-legacy-monitor` 关闭旧代码清理提醒。

历史目录与新目录命名错误要分开判断。已有来源记录优先；没有记录的非新版目录，用创建时间与切换时间 `enabled_at` 比较，切换前的目录不进入内部、不产生整理待办。这里不是监控修改时间，旧文件更新不会使目录变成新任务。有效新版优化必须先按编号识别，不得因为其创建时间早于 App 升级就漏掉待处理报表。旧监控仍由固定的 `aicnc_legacy_watch` 范围管理，日期过滤不扩大或清空该范围。`tests/test_aicnc_import.py` 覆盖未登记旧目录、已归档混单来源、新错放目录和切换前生成的有效新版优化。

这里的幂等性是“同一个优化编号重复操作不会再增加数量”。它不同于旧流程的文件指纹：新文件夹处理完后就不再看文件变化。SQLite 保存事实和处理结果，SwiftUI 只收集选择，Python 负责归属与分配校验。

有返工时，先保存确认方案，库存补单成功后再一次性记材料和消耗。未知库存结果通过 `recover_inventory_operation → aicnc_import.recover` 核对原单，不能为了恢复本地记录再次扣库。`production.cumulative_production_materials` 排除独立返工补单，避免下一次正常生产把已扣过的返工重新累计。

以下 7.1–7.5 的 XML/版本处理链保留给切换时固定的历史目录。相关旧代码仍有使用者，不在新版上线时提前删除；退出范围清空由 App 提醒安排删除。
...
布局上，新增表单固定在顶部，只有下方工厂单分组位于 `ScrollView` 中。标题与订单号同一行，标题栏和底部操作栏统一使用弹窗内的 `barHeight`（50 点）作为总高度，内容垂直居中，不再叠加纵向 padding。仅设置相同 padding 不能保证两栏等高，因为按钮与标题的固有布局高度不同。列表上下间距放在 `ScrollView` 外侧（顶部 14、底部 8），内部只保留水平边距，使滚动条轨道与列表可视区域一致。SwiftUI 中，内边距属于可滚动内容，外边距才会缩小滚动视口及其滚动条范围。商品结果与表单标签左边缘对齐；表头和内容使用相同列宽、居中对齐，原来的 98 点缩进分配给规格列。搜索结果每页完整显示一项，通过上一项/下一项浏览，长规格自然换行，避免固定行高裁切和嵌套滚动。翻页只改变浏览位置，已选 SKU 保持不变；更改查询会清空结果、选择和页码。新增卡片的顶部及两侧内边距为 16 点，底部为 8 点：数量行中的按钮有 44 点最小布局高度，实际控件轮廓外还有留白，因此上下视觉平衡不一定对应相同的 padding 数值。
...
`version` 是已读人工记录和工厂单状态的摘要，用来发现并发变化和阻止同一草稿重复新增，并非新的持久缓存。每条新增必须绑定本订单有效且未出库的工厂单；删除只允许当前订单的有效 manual 记录，保存时直接删除对应行。界面沿用后端 `editable` 权限：已出库或失效分组保留“仅查看”，不生成删除按钮，也不显示“操作”标题；空白列保留对齐。运行中的可编辑分组仍显示禁用按钮，暂时忙碌与业务只读分别处理。自动五金不变，不写 Traveler 或外部库存。保存后重新读取订单列表和详情。
...
### 助手订单卡片视觉细化（2026-09-21）

用户选定的局部效果图保留柔和浅色卡片，订单号采用独立编号，去掉胶囊底色；按后续反馈移除“订单”小标题，在 176 pt 身份区水平、垂直居中。`assistantOrderCard` 负责容器和身份区域；`assistantProgressRail` 用四等分列将名称、56 pt 圆角方形图标沿同一中心轴排列；节点中心固定在轨道 y=52 pt，连线按节点边缘留 10 pt 间距。图标采用原生 SF Symbols（分支、叠层、剪刀、货车），统一 28 pt medium；不将 AI 图稿作为业务界面的位图。

连线减为 2.5 pt，部分完成使用 7/9 pt 虚线节奏，移除原来的 9 pt 光晕底线。动画与减少动态效果分支保留，`assistantProgressSegmentState` 和四阶段数量来源均不变。`VStack` 的固定标题高度、间距和图标高度决定节点中心，修改这些值时必须一起核对连线 y 坐标。视觉样式与事实判定分开，避免更换外观时引入阶段串行锁定。订单行双击仍进入订单中心。

后续精简：阶段下方不再绘制数量，数量仍参与状态判断并保留在辅助阅读标签。轨道高度由 106 缩至 80 pt（标题 16 + 间距 8 + 图标 56），上下各 16 pt 留白后卡片为 112 pt；左侧分隔线缩至 62 pt。删除节点下方内容不改变节点中心 y=52，因此无需移动连线。

本轮只修改显示层，不包含数据库结构变更。源码中原有圆形 64 pt 图标、线段上下标签和粗线断言按用户确认方案更新；独立阶段状态、单独生产推进、零工厂单、双击入口、动画和局部滚动保护继续保留。最终完整门禁通过（422 Python，跳过 1；macOS UI、AIMES 离线、PP0067 workbook），一次串行构建通过。电脑控制工具明确禁止操作 Terminal，签名安装及实际视觉验收尚未完成；产物与证据另记于 `design-qa.md`，不以源码断言替代窗口验收。
...
“问题”是某次检查的结论，“业务事实”是已经确认发生的事情。两者分开后，用户在外部修复资料，App 再检查即可更新提示，不必维护长期的问题开关。

- 普通问题：界面 → `recheck-issue` → 针对来源校验或只读内存预览 → 成功后移除会话问题；失败保留提示。
- AIMES / Server：界面重新同步或扫描 → 最新来源结果 → 更新对应清单；任一来源失败单独显示未知。
- 库存恢复：界面 → `inventory recover-operation` → `verifyOutbound` 只读核对 → 原始操作计划提交本地事务 → 刷新看板。

SQLite 的 TEMP 表只属于当前连接，提交事务也不会把它变成主库持久表。常驻服务共享连接可以让用户在多个按钮之间继续处理同一批检查结果，但退出 App 后仍要重新检查。库存操作日志使用主库表，能跨重启恢复；恢复按钮不调用外部出库写入动作。

### 历史操作收尾与页面等待（2026-09-24）

操作日志的旧状态不一定等于当前业务状态。PP0072 的旧双单操作只有第一张响应，但本地两张单据已完整保存，第二张也有后续完成的操作记录。现在列表按原计划和明确单号只读检查这些证据；全部对应才消除提示，不删除历史记录，也不以“后来成功过”替代逐张比较。

JavaScript 的 `async` 函数返回 Promise（尚未完成的结果）。把它直接传给数组 `filter` 不会等待真假判断，Promise 会被当成真，从而选中错误页面。`waitForVisibleFrame` 改为逐个 `await predicate(frame)`，恢复核对还要等到目标单号和数量表头出现。离线回归模拟错误主页面、延迟加载及错误单号；它证明选择和等待逻辑，不能代替真实网站验收。

### 待处理空状态与 AIMES 日志措辞（2026-09-24）

顶部入口直接判断 `pendingCenterItems.isEmpty`；AIMES 历史记录在设置中查看，不作为当前待处理数量或入口显示依据。队列处理完后，关闭提示也只看当前队列。

AIMES 的进度文案在 `tools/aimes_lookup.mjs` 产生，直接记录动作，不写“内容已省略”等括号说明。`OperationLogReader.parse` 在读取旧日志时去掉已知占位说明，再按原规则脱敏；这属于显示处理，不重写日志文件，也不恢复敏感内容。

### AIMES 每日获取与待处理会话分开（2026-09-24）

启动 `loadOrderDashboardCache` → `syncDashboardAimes(force: false)` → `sync-aimes --aimes-if-needed` → 检查 `sync_runs` 中当天是否已有成功记录；成功过就不启动 AIMES 浏览器，当天只有失败记录仍会尝试。手动获取传入 `force: true`，不受当天成功记录限制。两种路径结束后，启动链都继续扫描 Server。

待处理提示不持久化不等于每次启动必须联网。现有 AIMES 缓存只保留有效身份，未保存的上一会话异常无法完整重建；重开跳过时明确说明使用本地数据，需查看外部最新状态可手动获取。这里不新增缓存、不恢复历史问题表。

## 零消耗生产：工厂单完成与订单领料分开（2026-09-24）

调用链：`ProductionSheet` 校验本次数量 → 全零时显示“仅确认生产，不扣减材料” → `AppModel.confirmProductionWithoutMaterials` → `order confirm-production-without-materials --confirm-write --materials-json` → `production.confirm_production_without_materials` → 一个 SQLite 事务保存生产时间及工厂单关联 → `list-index` 回读。

本次有正数量则保留原 `prepare-production` → 库存扣减流程。空白、负数和非有限输入不是零，前后端均阻断。订单材料余额与工厂单生产进度属于不同事实；本次无需新增领料仍可完成工厂单，也无需新增字段区分余料。零消耗路径不进入 `inventory.run_jdy`，原有材料、五金、订单级出库范围不变。

生产预览由弹窗的 `onAppear` 统一发起。订单中心按钮只打开弹窗，不提前读取；否则两次请求遇到串行执行保护时，第二次可能被丢弃，导致材料显示成功而弹窗就绪状态没有更新。`loadProductionPreview` 遇到忙碌明确回调失败，弹窗提供“重新读取材料”；成功回调才启用确认。回归必须挂载真实 SwiftUI 弹窗检查按钮状态，单独测试数量函数或后端不能覆盖这一时序问题。
