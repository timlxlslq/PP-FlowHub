...
- 当前 Git 共追踪 **344** 个文件；连同本次新增但可能尚未进入 Git 索引的文档，共登记 **350** 个文件。
...
| `macos/AssistantView.swift` | 助手页面、语音输入、任务队列和业务进度轨道。 | 70 |
| `macos/Info.plist` | macOS App 的权限声明、Bundle 配置和系统元数据。 | 0 |
| `macos/OperationLog.swift` | App 端操作日志读取、展示、脱敏和清理。 | 23 |
| `macos/OrderDashboardView.swift` | 订单看板、待处理中心、订单详情及生产/出库交互。 | 244 |
...
| `tests/test_order_index.py` | 自动化测试：验证 `order_index` 模块或业务场景。 | 104 |
...
| `traveler_assistant/database.py` | 中央 SQLite 路径、Schema、迁移和通用缓存读写。 | 12 |
...
| `traveler_assistant/order_index.py` | 订单索引核心：AIMES、Server、问题、确认、状态与同步证据。 | 207 |
