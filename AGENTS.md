...
- 真实业务数据库的结构变更必须先通知用户并取得明确确认，再执行。范围包括表、字段、字段类型、默认值、主键、外键、约束、索引、视图和触发器的新增、修改或删除，以及相关迁移。确认前先说明变更原因、具体结构差异、现有数据和业务流程的影响、备份与回滚方案；可以只读分析和准备供审阅的方案或迁移代码，但不得对真实业务数据库执行结构变更，也不得通过 App 启动、CLI、测试或安装流程隐式触发未确认的真实库迁移。一般功能修改授权、自动发布和长期安装授权不替代真实业务数据库结构变更确认；已确认范围内无需重复确认，范围变化须重新确认。
