...
- 真实业务数据库的结构变更必须在规划阶段提出并取得明确确认，然后再开始依赖该结构的功能实现及迁移代码修改；不得先完成代码修改、测试或构建，临近迁移或发布时才补问。范围包括表、字段、字段类型、默认值、主键、外键、约束、索引、视图和触发器的新增、修改或删除，以及相关迁移。规划时须说明变更原因、具体结构定义和与现状的差异、现有数据及业务流程的影响、备份与回滚方案，让用户在实施前审阅并确认。确认前可以只读分析、准备供审阅的结构方案和开展必要的隔离验证，但不能借隔离验证提前完成依赖未确认结构的功能实现；不依赖该结构的已授权工作可以继续。
- 真实库迁移只执行已确认的结构范围，不得通过 App 启动、CLI、测试或安装流程隐式触发未确认的迁移。一般功能修改授权、自动发布和长期安装授权不替代具体结构方案的确认；规划阶段已明确确认的范围，在实施、迁移和发布时无需重复确认。实施中发现需要改变已确认范围，应立即说明新增差异并在继续受影响的实现前重新取得确认，不得拖到发布前才补问。
- 测试数据、fixture 和与真实业务数据库隔离的测试数据库无需用户确认，可按任务需要直接创建、修改、重建、清理及执行结构迁移，用于开发和回归验证。执行前核对数据库路径与隔离配置，确保不会写入真实业务数据库或外部系统；测试库验证通过不代表已获准对真实业务数据库执行迁移。
...
测试安排先用几句话说明受影响行为、必要回归和完成条件，不为小改动新建测试计划。开发中选择能发现本次缺陷的最小测试；代码稳定后运行一次完整发布门禁，不先机械地把所有子门禁各跑一遍。通过后只有新增相关修改、失败或明确未解决风险才扩大或重复验证。新增测试应验证行为与错误边界，不为可逆低影响改动添加照抄实现的断言；不得为变绿删除、跳过或放宽仍有效的验收断言。失败先区分产品、测试和环境原因，有新证据或修复后再重试；完整门禁未通过时不得宣布发布验证完成。具体选择、串行限制和证据记录见发布文档。
...
版本号使用三段数字，标题栏显示 `PP FlowHub  V0.4.2` 这样的格式，不单独显示构建号。普通修改默认由 `build-app` 递增末位（如 `0.4.1 → 0.4.2`）；新增或修改较大功能时，Codex 根据范围改用 `./scripts/build-app --minor`，递增中间位并将末位归零（如 `0.4.2 → 0.5.0`）。构建脚本保存新版本，安装同一产物不再次递增；不要在构建前手动加一次版本导致重复升级。
...
复杂功能、跨模块修改、重大重构或明显架构取舍任一成立时才新建计划；简单修改直接实施。优先更新与当前任务相关的已有计划，确需新建时放在 `docs/plans/`，不依赖特定插件或技能。计划随任务更新，文首记录目标、状态、约束、步骤、验证、完成项和剩余决策，不为形式新建空目录。
